...
def require_exam_row(
    *,
    ...
    """依 exam_id 或 exam_page_id + course_id 載入 Exam（deleted=false）。person_id 非空時須一致。"""
    supabase = get_supabase()
    eid = int(exam_id or 0)
    et = (exam_page_id or "").strip()
    if eid > 0:
        q = (
            supabase.table("Exam")
        ...
        raise HTTPException(status_code=400, detail="請傳入 exam_id 或 exam_page_id")
