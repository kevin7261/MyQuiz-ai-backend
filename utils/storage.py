...
def generate_tab_id(person_id: str | None = None) -> str:
    """以 person_id 與目前電腦時間產生 tab_id（格式：{person_id}_yymmddhhmmss）。"""
...
def save_zip(
    ...
    上傳時可傳入 tab_id（由 API 指定）；repack/rag 可傳入 tab_id（依 rag_list 命名，如 220222_220301），未傳入則產生 UUID。
    上傳存於 storage/{person_id}/{tab_id}/upload/；無 person_id 時使用 storage/_/{tab_id}/upload/。
    repack/rag 存於 storage/{person_id}/{parent_tab_id}/repack/、storage/{person_id}/{parent_tab_id}/rag/，需傳入 person_id 與 parent_tab_id（上傳的 tab_id）。
    其他 API 可用 get_zip_path(tab_id) 取得檔案路徑後讀取。
    """
...
def get_zip_path(tab_id: str) -> Path | None:
    ...
    上傳：storage/{person_id}/{tab_id}/upload/{tab_id}.zip。
    repack/rag：storage/{person_id}/{parent_tab_id}/repack|rag/{tab_id}.zip。
    其他 API 可這樣使用：
        path = get_zip_path(tab_id)
    ...
    if folder is not None:
        if folder == FOLDER_UPLOAD:
            ...
            if person_id and parent_tab_id:
                ...
                path = _folder_dir(folder) / f"{tab_id}.zip"
        if path.exists():
            return path
    # 舊版：storage/{tab_id}/upload/
...
def get_zip_path_by_person(person_id: str, tab_id: str) -> Path | None:
    """
    依 person_id 與 tab_id 取得上傳 ZIP 路徑。上傳檔可能用原始檔名存於 upload/ 下。
    與 build-rag-zip 儲存位置一致。不存在則回傳 None。
    """
    if not tab_id or "/" in tab_id or "\\" in tab_id:
        return None
    pid = (person_id or "").strip() or UPLOAD_DEFAULT_PERSON
    ...
    if path.exists():
        return path
    # 相容舊資料：若無 metadata 或檔不存在，再試 tab_id.zip
    fallback = target_dir / f"{tab_id}.zip"
    return fallback if fallback.exists() else None


def get_tab_folder_path(person_id: str, tab_id: str) -> Path:
    """
    取得 storage/{person_id}/{tab_id} 資料夾路徑（該 tab_id 的 upload/repack/rag 皆在此下）。
    """
    if not tab_id or "/" in tab_id or "\\" in tab_id:
        raise ValueError("tab_id 不可包含路徑字元且不可為空")
    pid = (person_id or "").strip() or UPLOAD_DEFAULT_PERSON
...
def delete_tab_folder(person_id: str, tab_id: str) -> bool:
    ...
    僅在路徑位於儲存根目錄下時執行，避免路徑穿越。並清除 metadata 中該 tab_id 與其下 repack/rag 的紀錄。
    ...
    # 清除 metadata：該 tab_id（upload）與 parent_tab_id 為此 tab_id 的 repack/rag
    meta = _load_metadata()
