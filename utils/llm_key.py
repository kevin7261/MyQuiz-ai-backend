...
- RAG 出題／批改／弱點分析模型：key=rag-llm-model（GET/PUT /rag/llm_model；未設定時 fallback `QUIZ_LLM_MODEL`）
...
def get_rag_llm_model(course_id: int) -> str:
    ...
    stored = fetch_course_setting_value(COURSE_SETTING_RAG_LLM_MODEL, course_id)
