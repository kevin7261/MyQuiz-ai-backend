...
| GET | [`/rag/unit/youtube-url`](#get-ragunityoutube-url) | 解析 YouTube URL 與逐字稿 |
| **RAG 課程設定** | | |
...
列出 Rag（含 units→quizzes）。音訊單元（unit_type=3 且 mp3_file_name 非空）含 `mp3_audio_url`（指向 `GET /rag/page/unit/mp3-file`，不需 person_id，可直接作 `<audio src>`）；YouTube 單元（unit_type=4 且 youtube_url 非空）含 `youtube_url_api`（指向 `GET /rag/unit/youtube-url`）。
      ...
      "units": [
        {
          ...
          "youtube_url_api": "/rag/unit/youtube-url?rag_page_id=...&folder_name=...&person_id=...",
          "quizzes": [
            {
              "rag_quiz_id": 1,
              "rag_page_id": "string",
