...
- LLM API Key 存於 **`Course_Setting`**（依 `course_id`：`rag-api-key`、`exam-api-key`），以 GET/PUT `/rag/llm_api_key`、`/exam/llm_api_key` 管理。
...
├── routers/
│   ├── profile.py          # /profile — 登入、使用者 CRUD
...
| `exam-api-key` | Exam 出題／批改／個人弱點分析 OpenAI Key | GET/PUT `/exam/llm_api_key` |
| `person_analysis` 指令 | 個人弱點分析指令（`Person_Analysis`） | GET `/person-analysis/analysis`；寫入 GET/PUT `/rag/person_analysis_user_prompt_text` |
| `course_analysis` 指令 | 課程弱點分析指令（`Course_Analysis`） | GET `/course-analysis/analysis`；寫入 GET/PUT `/rag/course_analysis_user_prompt_text` |
...
```mermaid
flowchart LR
    A["上傳 ZIP<br/>POST /rag/page/upload-zip"] --> B["選擇單元<br/>unit_list"]
    B --> C{"unit_type?"}
...
- **個人**：設定與結果皆在 `Person_Analysis`（`person_id` varchar 登入帳號；課程共用指令 `person_id` 空字串）。`GET /person-analysis/analysis` 讀取；指令寫入 `/rag/person_analysis_user_prompt_text`；`GET /person-analysis/llm-analysis` 產生報告並寫入。
- **課程**：設定與結果皆在 `Course_Analysis`（`person_id` 空字串）。`GET /course-analysis/analysis` 讀取；指令寫入 `/rag/course_analysis_user_prompt_text`；`GET /course-analysis/llm-analysis` 彙整全課程已作答題目並產生報告寫入。
...
所有端點皆需 query 參數 `person_id`（RAG／Exam 相關端點另需 `course_id`；少數媒體端點如 `GET /rag/page/unit/mp3-file` 不需 `person_id`）。
...
RAG 與 Exam 採相同層級：**分頁（page）→ 單元（unit，僅 RAG）→ 題目（quiz：create → name → delete → 標記 → 出題 → 評分）→ 設定**。Swagger（`/docs`）路徑順序由 `utils/openapi_order.py` 統一排序。
...
| **帳號／個人檔案** | | |
| GET | [`/profile/users`](#get-profileusers) | 列出所有使用者 |
| POST | [`/profile/users`](#post-profileusers) | 新增使用者 |
| POST | [`/profile/users/batch`](#post-profileusersbatch) | 批次新增使用者 |
| PUT | [`/profile/users/delete`](#put-profileusersdelete) | 軟刪除使用者 |
| PATCH | [`/profile/password`](#patch-profilepassword) | 修改使用者密碼 |
| POST | [`/profile/login`](#post-profilelogin) | 登入 |
| PATCH | [`/profile`](#patch-profile) | 更新個人資料 |
| **RAG ZIP 管理** | | |
| GET | [`/rag/pages`](#get-ragpages) | 列出 Rag（含 units→quizzes） |
| POST | [`/rag/page/create`](#post-ragpagecreate) | 建立 Rag |
| POST | [`/rag/page/create-upload-zip`](#post-ragpagecreate-upload-zip) | 建立 Rag 並上傳 ZIP |
...
| POST | [`/rag/page/upload-zip`](#post-ragpageupload-zip) | 上傳 ZIP |
| POST | [`/rag/page/build-rag-zip`](#post-ragpagebuild-rag-zip) | 打包 RAG ZIP（NDJSON 串流） |
| GET | [`/rag/page/units`](#get-ragpageunits) | 列出 Rag_Unit（含 quizzes） |
| PUT | [`/rag/page/unit/unit-name`](#put-ragpageunitunit-name) | 更新 Rag_Unit unit_name |
| GET | [`/rag/page/unit/mp3-file`](#get-ragpageunitmp3-file) | 取得音訊（unit_type=3） |
| GET | [`/rag/page/unit/youtube-url`](#get-ragpageunityoutube-url) | 取得 YouTube URL（unit_type=4） |
| POST | [`/rag/page/unit/quiz/create`](#post-ragpageunitquizcreate) | 新增空白 Rag_Quiz |
...
| POST | [`/rag/page/unit/quiz/llm-generate-db`](#post-ragpageunitquizllm-generate-db) | LLM 出題（沿用 DB prompt） |
| POST | [`/rag/page/unit/quiz/llm-generate-followup`](#post-ragpageunitquizllm-generate-followup) | LLM 追問出題 |
| POST | [`/rag/page/unit/quiz/llm-generate-followup-db`](#post-ragpageunitquizllm-generate-followup-db) | LLM 追問出題（沿用 DB prompt） |
| POST | [`/rag/page/unit/quiz/llm-grade`](#post-ragpageunitquizllm-grade) | 非同步評分（202 + job_id） |
| POST | [`/rag/page/unit/quiz/llm-grade-db`](#post-ragpageunitquizllm-grade-db) | 非同步評分（沿用 DB prompt） |
...
| GET | [`/rag/person_analysis_user_prompt_text`](#get-ragperson_analysis_user_prompt_text) | 取得個人分析 prompt（`Person_Analysis`） |
| PUT | [`/rag/person_analysis_user_prompt_text`](#put-ragperson_analysis_user_prompt_text) | 寫入個人分析 prompt（`Person_Analysis`） |
...
| POST | [`/exam/page/create`](#post-exampagecreate) | 建立 Exam |
...
| POST | [`/exam/page/quiz/create`](#post-exampagequizcreate) | 新增空白 Exam_Quiz |
...
| POST | [`/exam/page/quiz/create-llm-generate`](#post-exampagequizcreate-llm-generate) | 建立並 LLM 出題 |
| POST | [`/exam/page/quiz/create-llm-generate-followup`](#post-exampagequizcreate-llm-generate-followup) | 建立並 LLM 追問出題 |
...
| GET | [`/exam/llm_api_key`](#get-examllm_api_key) | 讀取 exam-api-key |
| PUT | [`/exam/llm_api_key`](#put-examllm_api_key) | 寫入 exam-api-key |
| **弱點分析** | | |
| GET | [`/person-analysis/analysis`](#get-person-analysisanalysis) | 讀取最新 `Person_Analysis` |
| GET | [`/person-analysis/llm-analysis`](#get-person-analysisllm-analysis) | 個人作答分析（含弱點報告） |
| GET | [`/course-analysis/analysis`](#get-course-analysisanalysis) | 讀取最新 `Course_Analysis` |
| GET | [`/course-analysis/llm-analysis`](#get-course-analysisllm-analysis) | 課程作答分析（含弱點報告） |
| **Prompt 模板** | | |
...
修改指定使用者密碼。Body 僅 `person_id`（要修改的帳號）、`password`（新密碼）。需帶 query `person_id`。
...
成功時回傳與登入相同格式（`user`、`courses`，不含 password）。
      ...
      "units": [
        {
          ...
          "updated_at": "2024-01-01T00:00:00+08:00",
          "created_at": "2024-01-01T00:00:00+08:00",
          "mp3_audio_url": "/rag/page/unit/mp3-file?rag_page_id=...&rag_unit_id=...&course_id=...",
          "youtube_url_api": "/rag/page/unit/youtube-url?rag_page_id=...&rag_unit_id=...&course_id=...",
          "quizzes": [
            {
              "rag_quiz_id": 1,
              "rag_page_id": "string",
...
#### `POST /rag/page/create-upload-zip`
...
#### `POST /rag/page/unit/quiz/create`
...
軟刪除 Exam_Quiz（`deleted=true`）；並一併軟刪除 `follow_up_exam_quiz_id` 指向該題的追問子題鏈。`GET /exam/pages` 等僅回傳未刪除題（`deleted=false` 或 `null`）。
...
不呼叫 LLM；自 `Person_Analysis` 讀取最新一筆。必填 query：`person_id`（登入帳號如 `student01`；**課程共用指令傳空字串**）、`course_id`。
...
```json
{
  ...
  "person_id": "student01",
  "course_id": 2,
  "analysis_prompt_text": "教師指令純文字（僅 API PUT 寫入之指令列）",
...
> 無紀錄時各欄位為 `null`。DB `person_id` 須為 `varchar(255)`（見下方 DDL）。

#### `GET /person-analysis/llm-analysis`

依學生 `person_id`、`course_id` 取得已作答 Exam_Quiz 並產生弱點報告。教師指令自 `Person_Analysis`（該生優先，其次 `person_id` 空字串之課程共用列）。LLM 成功時寫入新列（僅 `analysis_text` 為報告；`analysis_prompt_text` 僅存於教師指令列）。
  ...
  "exams": [
    {
      ...
      "tab_name": "string",
      "person_id": "string",
      "quizzes": [ /* Exam_Quiz[]（含 answer_content 非空者），結構同 GET /exam/pages */ ]
    }
  ],
  "count": 1,
...
> `weakness_report` 為 LLM `message.content` 原文 Markdown；無 **exam-api-key**、無可分析題目或失敗時為 null。`analysis_llm_model` 為本次弱點分析實際使用的模型（`Course_Setting` key=`llm-model`；未設定時為程式預設 `gpt-5.4`）。

#### `GET /course-analysis/analysis`

不呼叫 LLM；自 `Course_Analysis` 讀取最新一筆。必填 query `course_id`。

```json
{
  "course_analysis_id": 1,
  "course_id": 2,
  "analysis_user_prompt_text": "教師指令純文字",
  "analysis_prompt_text": "教師指令純文字（與 analysis_user_prompt_text 同源）",
...
#### `GET /course-analysis/llm-analysis`

依 `course_id` 取得全課程已作答 Exam_Quiz 並產生弱點報告。教師指令自 `Course_Analysis`。LLM 成功時寫入新列（僅 `analysis_text` 為報告；`analysis_prompt_text` 僅存於教師指令列）。
...
取得課程共用個人分析指令（`Person_Analysis`，`person_id` 空字串）。必填 query `course_id`。與 `GET /person-analysis/analysis?person_id=&course_id=` 同源。
...
寫入課程共用個人分析指令至 `Person_Analysis`（僅該課程 `user_type` 1／2）。必填 query `course_id`。
...
取得課程分析指令（`Course_Analysis`，`person_id` 空字串）。必填 query `course_id`。與 `GET /course-analysis/analysis?course_id=` 同源。
...
寫入課程分析指令至 `Course_Analysis`（僅該課程 `user_type` 1／2）。必填 query `course_id`。
