...
MyQuiz.ai 是一套 **AI 輔助測驗與學習分析** 系統。教師上傳課程教材（Office、PDF、Markdown、音訊、YouTube 逐字稿等），後端依單元類型建置 FAISS 向量庫或逐字稿 ZIP，再透過 OpenAI GPT-4o 自動出題、批改與產生弱點報告；學生則在 Exam 模組完成測驗並取得 AI 評語。
...
- LLM API Key 存於 **`Course_Setting`**（依 `course_id`：`rag-api-key`、`exam-api-key`），以 GET/PUT `/rag/api_key`、`/exam/api_key` 管理。
...
```
{person_id}/{rag_tab_id}/upload/{rag_tab_id}.zip   ← 原始上傳 ZIP
{person_id}/{rag_tab_id}/repack/{unit_id}.zip      ← 單元 repack
{person_id}/{rag_tab_id}/rag/{unit_id}_rag.zip     ← FAISS 或 transcript ZIP
```

根目錄 `_metadata.json` 記錄 tab_id → 路徑對照，供 `get_zip_path()` 查詢。
...
erDiagram
    ...
    Rag {
        int rag_id PK
        string rag_tab_id UK
        int course_id
        json file_metadata
    }
    Rag_Unit {
        int rag_unit_id PK
        string rag_tab_id FK
        int unit_type
        string transcript
    ...
    Exam {
        int exam_id PK
        string exam_tab_id UK
        int course_id
    }
    Exam_Quiz {
        int exam_quiz_id PK
        string exam_tab_id FK
        int rag_quiz_id FK
        int quiz_rate
        string answer_content
        string answer_critique
...
| key | 用途 | 管理端點 |
|-----|------|----------|
| `rag-api-key` | RAG 出題／批改 OpenAI Key | GET/PUT `/rag/api_key` |
...
flowchart TD
    ...
    FAISS --> LLM["GPT-4o JSON 出題<br/>quiz_content / hint / reference"]
    TR --> LLM
...
| **RAG 課程設定** | | |
| GET | [`/rag/api_key`](#get-ragapi_key) | 讀取 rag-api-key（user_type 1/2） |
| PUT | [`/rag/api_key`](#put-ragapi_key) | 寫入 rag-api-key |
...
| LLM 模型 | 出題／批改／分析皆預設 `gpt-4o`；embedding `text-embedding-3-small` |
