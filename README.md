...
| GET | [`/rag/unit/mp3-file`](#get-ragunitmp3-file) | 取得原始音訊（binary） |
| GET | [`/rag/unit/youtube-url`](#get-raguniityoutube-url) | 解析 YouTube URL |
| **測驗** | | |
...
回傳文字單元（unit_type=2）的 text_file_name 與 transcription。不需 person_id。
...
自 upload ZIP 回傳原始音訊 bytes（binary response，非 JSON）。

- Content-Type：`audio/mpeg`（依副檔名決定）
...
自 upload ZIP 解析 YouTube URL。需 person_id。
