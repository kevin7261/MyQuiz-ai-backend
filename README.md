...
| Quiz 測驗 | `/v1/quiz` | 搭配 Bank 的測驗：快照題組、逐題出題、作答批改、課程內容追問 |
...
Bank → Bank_Group（題組連續出題） → Quiz（測驗＋課程內容追問）
```
...
| `quiz-analysis-api-key` | 測驗弱點分析 | `/v1/quiz-analyses/llm-api-key` |
...
| `quiz-analysis-llm-model` | 測驗弱點分析模型 | `/v1/quiz-analyses/llm-model` |
...
|-----------|------|----------|
| 1 | 管理者 | 全部；可建 FAISS |
| 2 | 教師 | 設 API Key、模型、分析指令、成員管理 |
| 3 | 學生 | 作答、查詢；不能改設定 |

需 **1 或 2** 才能：讀寫各模組 API Key／模型、分析指令、課程成員管理。
...
| `Quiz` / `Quiz_Group` / `Quiz_QA` | 測驗、題組快照、測驗題目 |
| `Quiz_Ask` | 測驗題組的「課程內容追問」紀錄 |
...
| `quiz-analysis-api-key` | 測驗弱點分析 | Quiz_Analysis |
...
| `quiz-analysis-llm-model` | 測驗分析模型 | Quiz_Analysis |
...
| `quiz_analysis_user_prompt_text` | 測驗分析規則 | Quiz_Analysis |
...
| F Quiz 測驗 | 快照 Bank 題組成測驗，逐題出題、作答、追問課程內容 | 5.6 |
...
3. `POST /v1/bank/pages/{page}/units/{unit}/groups` — 建題組，設定：
   ...
   - 三個 prompt 一律由前端輸入，留空即存空字串，後端不提供預設
...
7. 題組設 `for_exam=true` 後可供 Quiz 測驗選用
...
| prompt 來源 | 每題 body／DB | 題組設定（前端輸入，留空即空、無預設） |
...
### 5.6 流程 F：Quiz 測驗（快照＋追問課程內容）

**目的**：Quiz 之於 Bank，等於 Exam 之於 RAG——「正式應試」層。把 Bank 題組**快照**成測驗題組，逐題出題、作答批改，還能對課程內容**發問**（Quiz_Ask）。

**資料階層**

```
Quiz（測驗）─< Quiz_Group（自 Bank_Group 快照）─< Quiz_QA（題目）
                └─< Quiz_Ask（課程內容追問紀錄）
```
...
2. `POST /v1/quiz/pages` — 建測驗
3. `GET /v1/quiz/bank-groups` — 列出可選的 Bank 題組（`for_exam=true`）
4. `POST /v1/quiz/pages/{id}/groups` — 把選定的 Bank_Group **快照**成 Quiz_Group（prompt、`qa_count`、模型、單元資訊全部凍結；之後改 Bank 不影響測驗）
...
![圖 22：Quiz 測驗流程](docs/images/img-22.jpg)
...
> bank／quiz 的 `/llm-api-key`、`/llm-model` 端點同形，由 `routers/llm_settings.py` 的 factory 依參數產生，避免兩份檔案平行維護。
...
| **Quiz 測驗** | | |
| GET | `/v1/quiz/pages` | 列出測驗（含 quiz_groups→qas 巢狀） |
| POST | `/v1/quiz/pages` | 建立測驗（201） |
| PATCH | `/v1/quiz/pages/{quiz_page_id}` | 更新測驗 tab_name |
| DELETE | `/v1/quiz/pages/{quiz_page_id}` | 軟刪除測驗 |
| GET | `/v1/quiz/bank-groups` | 列出可加入測驗的 Bank 題組（for_exam=true） |
| **Quiz 題組與題目** | | |
...
| **測驗弱點分析** | | |
| GET | `/v1/quiz-analyses` | 列出測驗所有結果列 |
| POST | `/v1/quiz-analyses` | 新增空白分析列（201） |
| POST | `/v1/quiz-analyses/{id}/llm-analysis` | 產生測驗弱點報告寫入該列 |
...
| GET/PUT | `/v1/quiz-analyses/analysis-user-prompt-text` | 讀寫測驗分析規則課程預設 |
| **Prompt 模板** | | |
...
> - 三個 prompt 一律由前端輸入，直接存 body 送的值；送空字串就存空字串，後端不提供預設。
...
Body：`{ "for_exam": true }`。owner 限定。設 true 後此題組可被 Quiz 測驗選用。回傳 Bank_Group 整列。
...
> 題組三個 prompt 一律由前端在建題組 body 或 per-group `PUT /v1/bank/groups/{id}/…-prompt-text` 端點輸入，留空即存空字串，後端不提供課程預設。

---

### Quiz 測驗 `/v1/quiz`

> Quiz 之於 Bank ＝ Exam 之於 RAG。測驗題組為 Bank_Group 的**快照**，逐題出題、作答批改、可對課程內容追問。逐欄位文件見 [docs/QUIZ_API.md](docs/QUIZ_API.md)。
...
列出**呼叫者擁有**的測驗，每筆帶 `quiz_groups[]`（每組帶 `qas[]`，依 `question_series_index` 升序）。
...
更新測驗 tab_name（owner 限定）。Body：`{ "tab_name": "新名稱" }`。
...
軟刪除測驗（不動其 Quiz_Group／Quiz_QA）。owner 限定。
...
列出**可加入測驗**的 Bank_Group（`for_exam=true`、未刪除；不限擁有者），附其單元資訊：
...
挑選一個 Bank_Group，**快照**成此測驗下的 Quiz_Group（**不呼叫 LLM**），回 **201**。Body：

```json
{ "bank_group_id": 1, "group_name": "" }
```

> 快照凍結：三種 prompt、`qa_count`、`question_llm_model`／`answer_llm_model`、單元資訊（`unit_name`／`unit_type`）與來源鍵（`bank_page_id`／`bank_unit_id`／`bank_group_id`）。**之後改 Bank 不影響已建立的測驗。**`group_name` 留空沿用 Bank_Group 名稱。
