...
| `llm-model` | RAG／Exam 出題批改、弱點分析共用模型（預設 `gpt-5.4`） | `/v1/rag/llm-model` |
...
|-----------|------|----------|
| 1 | 開發者 | 全部；可建 FAISS |
| 2 | 管理者 | 設 API Key、模型、prompt 預設、成員管理 |
| 3 | 學生 | 作答、查詢；不能改設定 |
...
```
MyQuiz-ai-backend/
├── main.py                  # 入口：CORS、APILogMiddleware、掛 13 個 router
├── routers/
...
│   ├── exam/                # Exam 測驗
│   ├── person_analysis.py   # 個人弱點分析
│   ├── course_analysis.py   # 課程弱點分析
...
輪詢批改結果，語意同 [RAG answer-result](#get-v1ragquizzesanswer-resultjob_id)。`ready` 時 `result` 含 `quiz_comments`（Markdown 字串陣列）、`bank_qa_id`，並另附 `bank_qa`（Bank_QA 整列）。LLM 失敗 → `status="ready"` + 頂層 `llm_error`。查無 job → 404（請重新送出批改）。
