...
flowchart TB
    ...
    subgraph Backend["MyQuiz.ai Backend — FastAPI（/v1）"]
        ...
        ROUTERS["Routers 層<br/>zip / answer / exam / …"]
        SERVICES["Services 層"]
        UTILS["Utils 層"]
    end
    ...
    subgraph ServicesDetail["Services"]
        ...
        GR["answering<br/>LLM 批改 + FAISS 檢索"]
...
sequenceDiagram
    ...
    alt LLM 批改（非同步）
        RT->>FE: 202 + job_id
        RT->>SV: BackgroundTasks run_answer_job_background
        SV->>LLM: Chat Completions (JSON)
        SV->>DB: 寫入 answer_critique
        FE->>RT: GET answer-result/{job_id} 輪詢
    end
    RT->>MW: 回應
...
路由掛載順序（`main.py`，皆掛 `/v1`）：`zip` → `answer` → `exam` → `person_analysis` → `course_analysis` → `profile` → `college` → `course` → `course_settings` → `prompt` → `log`。Swagger（`/docs`）路徑順序由 `utils/openapi_order.py` 統一排序，與掛載順序無關。
...
│   │   └── routes.py         #   端點
│   ├── answer/                # /v1/rag — LLM 出題、批改、旗標、API Key、模型設定
│   │   ├── schemas.py
│   │   ├── helpers.py        #   _answer_job_results（記憶體 job 字典）等
│   │   └── routes.py
...
│   ├── answering.py            # LLM 批改、Background Job
...
| `person_analysis_user_prompt_text` | 個人弱點分析指令（教師規則） | GET/PUT `/v1/rag/person-analysis-user-prompt-text` |
| `course_analysis_user_prompt_text` | 課程弱點分析指令（教師規則） | GET/PUT `/v1/rag/course-analysis-user-prompt-text` |

> 弱點分析**指令**亦存於 `Course_Setting`（依 `course_id`，課程層級共用）；`Person_Analysis`／`Course_Analysis` 只放**結果列**（一列＝一筆分析紀錄），其 `analysis_prompt_text` 僅為產生報告當下的規則快照。
...
| LLM 操作保留 `llm-` 前綴 | `llm-generate`、`llm-answer`、`llm-analysis` |
...
- `POST /v1/exam/quizzes/answer` ＝ `POST /v1/exam/quizzes/llm-answer`
...
```mermaid
stateDiagram-v2
    [*] --> Pending: POST llm-answer → 202 job_id
    ...
    Ready --> [*]: GET answer-result status=ready
    ReadyLlmError --> [*]: GET answer-result status=ready + llm_error
    Error --> [*]: GET answer-result status=error
    Pending --> Pending: GET answer-result status=pending
```

1. `POST .../llm-answer` 立即回 **HTTP 202** + `{ "job_id": "uuid", "answer_llm_model": "…" }`。
2. 背景執行 `run_answer_job_background`：unit_type 2/3/4 走逐字稿路徑，unit_type 1 走 FAISS 檢索（以**題幹**為 query，k=5；temperature 0.3）。
3. LLM 回傳 JSON → 解析 `quiz_comments` → 以 `\n\n` 合併寫入 `answer_critique`（純文字）。
4. 前端輪詢 `GET .../answer-result/{job_id}` 直到 `status` 為 `ready` 或 `error`；`ready` 時若 `llm_error` 非 null 表示 LLM 呼叫失敗（`quiz_comments` 為空陣列）。
5. Job 結果存於**程序記憶體**（`routers/answer/helpers.py` `_answer_job_results` 字典；重啟後遺失）；查無 job_id 時回 **404**，前端應重新送出評分。

### 弱點分析流程

| | 個人分析 | 課程分析 |
|---|---------|---------|
| 資料表（結果列） | `Person_Analysis` | `Course_Analysis` |
| API Key | `exam-api-key` | `rag-api-key` |
| 指令儲存（Course_Setting key） | `person_analysis_user_prompt_text` | `course_analysis_user_prompt_text` |
...
| 讀取結果 | GET `/v1/person-analyses`（列出自己所有結果列） | GET `/v1/course-analyses`（列出課程所有結果列） |
| 產生報告 | **POST** `/v1/person-analyses/{id}/llm-analysis` | **POST** `/v1/course-analyses/{id}/llm-analysis` |
| 分析範圍 | 該生已作答 Exam_Quiz | 全課程已作答題目 |

與測驗頁同一套「一列一 page、按 id 操作」模式：

1. `POST /v1/person-analyses`（query 可帶 `analysis_name`）建立空白結果列（`analysis_text=''`），取得主鍵。
2. `POST /v1/person-analyses/{id}/llm-analysis` 產生報告並**按主鍵寫入該列**（驗證該列屬於呼叫者且 `course_id` 相符；同時將當次指令快照寫入該列 `analysis_prompt_text`，並回傳 `analysis_llm_model`）。
3. `PATCH`／`DELETE /{id}` 改名／軟刪除，永遠只動指定列，不會新增列。

- 指令來源：`Course_Setting` 對應 key（依 `course_id`，課程層級共用）；未設定時以「（未提供）」帶入模板。
...
| 批改 job | `answer-result` 回 `status="ready"` + 頂層 `llm_error`，`result.quiz_comments` 為空陣列 |
...
除 `POST /v1/auth/login` 與 RAG 單元媒體端點（`/v1/rag/units/{id}/*`）外，所有端點皆需 `Authorization: Bearer <token>` 標頭；RAG／Exam／分析（llm-analysis、POST）／Log 端點另需 query 參數 `course_id`。
