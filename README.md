...
| `person_analysis` 指令 | 個人弱點分析指令（`Person_Analysis_Setting`） | GET/PUT `/person-analysis/analysis` 或 GET/PUT `/rag/person_analysis_user_prompt_text` |
...
- **個人**：設定與結果皆在 `Person_Analysis_Setting`（`person_id` varchar 登入帳號；課程共用指令 `person_id` 空字串）。`PUT/GET /person-analysis/analysis`；`GET /person-analysis/llm-analysis` 產生報告並寫入。
...
| GET | [`/rag/person_analysis_user_prompt_text`](#get-ragperson_analysis_user_prompt_text) | 取得個人分析 prompt（`Person_Analysis_Setting`） |
| PUT | [`/rag/person_analysis_user_prompt_text`](#put-ragperson_analysis_user_prompt_text) | 寫入個人分析 prompt（`Person_Analysis_Setting`） |
...
| **弱點分析** | | |
| GET | [`/person-analysis/analysis`](#get-person-analysisanalysis) | 讀取最新 `Person_Analysis_Setting` |
| PUT | [`/person-analysis/analysis`](#put-person-analysisanalysis) | 寫入教師分析指令（純文字） |
...
不呼叫 LLM；自 `Person_Analysis_Setting` 讀取最新一筆。必填 query：`person_id`（登入帳號如 `student01`；**課程共用指令傳空字串**）、`course_id`。
...
```json
{
  ...
  "person_id": "student01",
  "course_id": 2,
  "analysis_prompt_text": "教師指令純文字，或 LLM 後之 JSON",
...
#### `PUT /person-analysis/analysis`

寫入教師分析指令（`analysis_prompt_text` 純文字，不呼叫 LLM）。必填 query：`person_id`（課程共用傳空字串）、`course_id`；body：`{"analysis_prompt_text":"…"}`。僅開發者／管理者可寫入。取代舊 `PUT /rag/person_analysis_user_prompt_text`（Course_Setting）。

#### `GET /person-analysis/llm-analysis`

依學生 `person_id`、`course_id` 取得已作答 Exam_Quiz 並產生弱點報告。教師指令自 `Person_Analysis_Setting`（該生優先，其次 `person_id` 空字串之課程共用列）。LLM 成功時寫入新列（`analysis_prompt_text` 為送交 LLM 之 JSON、`analysis_text` 為報告）。
...
> `weakness_report` 為 LLM `message.content` 原文 Markdown；無 API Key、無可分析題目或失敗時為 null。`analysis_llm_model` 為本次弱點分析實際使用的模型（`Course_Setting` key=`llm-model`；未設定時為程式預設 `gpt-5.4`）。
...
取得課程共用個人分析指令（`Person_Analysis_Setting`，`person_id` 空字串）。必填 query `course_id`。與 `GET /person-analysis/analysis?person_id=&course_id=` 同源。
...
寫入課程共用個人分析指令至 `Person_Analysis_Setting`（僅該課程 `user_type` 1／2）。必填 query `course_id`。
