...
class ExamCreateLlmGenerateQuizFollowupRequest(BaseModel):
    ...
    quiz_history_list: list[ExamQuizHistoryPair] = Field(
        default_factory=list,
        description="先前問答（物件陣列）；未傳時自 Exam_Quiz.quiz_history_list 讀取",
...
def _prewrite_exam_quiz_history_list(
    supabase: Any, *, exam_quiz_id: int, qa_dicts: list[dict[str, str]]
) -> None:
...
def _exam_llm_generate_quiz_impl(
    *,
    ...
    request_qa = _exam_quiz_history_qa_dicts(quiz_history_qa or []) if followup else None
    qa_dicts, stems_for_llm = resolve_quiz_history_for_generate(
        ...
        db_raw=qrow.get("quiz_history_list"),
    )
