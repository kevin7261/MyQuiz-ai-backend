...
- POST /exam/generate-quiz：依 exam_tab_id 與 rag_name 查找 RAG ZIP 出題，寫入 Exam_Quiz（exam_id, exam_tab_id）。
...
class ExamGenerateQuizRequest(BaseModel):
    ...
    system_prompt_instruction: str = Field(..., description="出題系統指令（Exam 表無此欄，必填）")
...
def exam_generate_quiz(body: ExamGenerateQuizRequest):
    """
    傳入 exam_tab_id（create-exam 的 exam_tab_id）與 rag_name，程式以 {rag_name}_rag 查找 RAG ZIP 出題。
    llm_api_key、system_prompt_instruction 由請求 body 傳入。出題成功後寫入 public.Exam_Quiz 表（exam_id, exam_tab_id）。
