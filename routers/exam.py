...
class ExamQuizGradeRequest(BaseModel):
    """POST /exam/tab/quiz/llm-grade：欄位順序對齊 public.Exam_Quiz（題幹優先於作答）；
    批改用之 quiz_user_prompt_text／answer_user_prompt_text 優先採 Exam_Quiz 列（出題成功時寫入之快照），缺則自 Rag_Quiz 補齊。"""
...
出題成功後更新該筆 Exam_Quiz（`rag_tab_id`、`quiz_name`、quiz_content／quiz_hint／quiz_answer_reference、rag_unit_id、rag_quiz_id；自該 `rag_quiz_id` 之 Rag_Quiz 寫入 `quiz_user_prompt_text`、`answer_user_prompt_text` 以記錄當下模板；清空作答欄位）。
"""
...
def exam_llm_generate_quiz(request: Request, body: ExamLlmGenerateQuizRequest, caller_person_id: PersonId):
    ...
    rq_one = (
        supabase.table("Rag_Quiz")
        .select("rag_tab_id, quiz_user_prompt_text, answer_user_prompt_text")
...
async def exam_grade_submission(
    ...
    qsel = (
        supabase.table("Exam_Quiz")
        .select(
            "exam_quiz_id, exam_tab_id, unit_name, rag_tab_id, rag_unit_id, rag_quiz_id, person_id, quiz_content, "
            "quiz_user_prompt_text, answer_user_prompt_text"
        )
        .eq("exam_quiz_id", body.exam_quiz_id)
        .limit(1)
        .execute()
