...
→ POST tab/quiz/llm-grade → GET tab/quiz/grade-result/{job_id} → POST tab/quiz/quiz-rate → POST tab/quiz/grade-rate
...
class ListExamResponse(BaseModel):
    """GET /exam/tabs 回應：每筆 Exam 含 quizzes[]（Exam_Quiz，含 follow_up 鏈、quiz_history_list、quiz_rate、grade_rate）。"""
...
class ExamQuizRateRequest(BaseModel):
    """POST /exam/tab/quiz/quiz-rate：更新 Exam_Quiz.quiz_rate。"""
...
class ExamQuizGradeRateRequest(BaseModel):
    """POST /exam/tab/quiz/grade-rate：更新 Exam_Quiz.grade_rate。"""
    exam_quiz_id: int = Field(..., ge=1, description="Exam_Quiz 主鍵")
...
@router.post("/tab/quiz/quiz-rate", summary="Exam Rate Quiz", status_code=200)
...
@router.post("/tab/quiz/grade-rate", summary="Exam Rate Grade", status_code=200)
