...
def _load_exam_for_quiz(
    ...
    *,
    ...
    if exam_id > 0:
        exam_rows = (
            supabase.table("Exam")
            .select("exam_id, exam_tab_id, tab_name, person_id, local, deleted")
            .eq("exam_id", exam_id)
            .eq("deleted", False)
            .execute()
        )
...
def list_rag_for_exams(
    ...
    def _list_rag_for_exams_once() -> ListRagForExamsResponse:
        ...
        quizzes_resp = (
            ...
            .select(
                "rag_quiz_id, rag_tab_id, rag_unit_id, person_id, quiz_name, quiz_user_prompt_text, "
...
def exam_llm_generate_quiz(request: Request, body: ExamLlmGenerateQuizRequest, caller_person_id: PersonId):
    ...
    qsel = (
        ...
        .select(
            "exam_quiz_id, exam_tab_id, rag_tab_id, rag_unit_id, rag_quiz_id, person_id, unit_name, quiz_name"
        )
