...
class CreateExamRequest(BaseModel):
    """POST /exam/tab/create：欄位同 public.Exam（exam_tab_id, tab_name, person_id, local；不含 exam_id／deleted／時間戳）。"""
...
class ExamLlmGenerateQuizRequest(BaseModel):
    ...
    選填 `quiz_history_list`：已出過題目題幹，傳入 `utils.quiz_generation.generate_quiz*` 以避免重複。"""
...
def create_exam(body: CreateExamRequest, caller_person_id: PersonId, course_id: CourseId):
...
def update_exam_unit_tab_name(body: UpdateExamUnitNameRequest, caller_person_id: PersonId, course_id: CourseId):
...
def exam_insert_empty_quiz(body: ExamCreateQuizRequest, caller_person_id: PersonId, course_id: CourseId):
...
def exam_llm_generate_quiz(request: Request, body: ExamLlmGenerateQuizRequest, caller_person_id: PersonId, course_id: CourseId):
    ...
    try:
        ...
        try:
            ...
            _logger.exception("POST /exam/tab/quiz/llm-generate 寫入 Exam_Quiz 失敗")
...
def update_exam_quiz_rate(body: ExamQuizRateRequest, caller_person_id: PersonId, course_id: CourseId):
