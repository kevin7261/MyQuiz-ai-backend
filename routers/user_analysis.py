...
- 分析指令存 Course_Setting（GET/PUT /user-analyses/analysis-user-prompt-text）；
...
class UserAnalysisListItem(BaseModel):
    ...
    analysis_user_prompt_text: Optional[str] = Field(
        "", description="該次產生報告時使用者輸入的分析指令快照"
    )
...
def list_user_analyses(person_id: PersonId, course_id: CourseId):
    ...
    try:
        ...
        logger.exception("GET /user-analyses 失敗 course_id=%s", course_id)
...
def add_user_analysis(
    ...
    try:
        ...
        logger.exception("POST /user-analyses 失敗 course_id=%s", course_id)
...
def update_user_analysis_name_endpoint(
    ...
    try:
        ...
        logger.exception(
            "PATCH /user-analyses/%s 失敗", user_analysis_id
        )
...
def delete_user_analysis(
    ...
    try:
        ...
        logger.exception(
            "DELETE /user-analyses/%s 失敗", user_analysis_id
        )
...
def user_llm_analysis(
    ...
    try:
        ...
        logger.exception(
            "POST /user-analyses/%s/llm-analysis 失敗", user_analysis_id
        )
