"""routers.bank routes（自 routers.zip routes 複製，Bank/Bank_Unit 版；不含 quiz）。"""
...
from utils.bank_llm_key import get_bank_api_key
...
from .schemas import (
    ListBankResponse,
    PackRequest,
    BankUnitMp3FilePreviewResponse,
    BankUnitMp3FileResponse,
    BankUnitTextPreviewResponse,
    BankUnitTextResponse,
    BankUnitYoutubeUrlPreviewResponse,
    BankUnitYoutubeUrlResponse,
    UpdateBankUnitNameRequest,
)
from .group_helpers import groups_by_bank_unit_ids
from .helpers import (
    BANK_UNIT_TYPE_MP3,
    BANK_UNIT_TYPE_RAG,
    BANK_UNIT_TYPE_TEXT,
    ...
    _create_bank_record,
    _do_delete_bank_file_by_page_id,
    ...
    _units_by_bank_page_ids,
    _upload_bank_zip_contents,
    _validate_bank_tab_create_fields,
)
        ...
        for row in data:
            ...
            for unit in units:
                ...
                if (
                    ...
                    unit["mp3_audio_url"] = (
                        f"/bank/units/{uid_int}/mp3-file?"
                        + urlencode(
                            {
                                "bank_page_id": str(page_id).strip(),
                                "course_id": str(course_id),
                            }
                        )
                    )
                if (
                    utype == BANK_UNIT_TYPE_YOUTUBE
                    and (unit.get("youtube_url") or "").strip()
                    and page_id
                    and uid_int is not None
                ):
                    unit["youtube_url_api"] = (
                        f"/bank/units/{uid_int}/youtube-url?"
                        + urlencode(
                            {
                                "bank_page_id": str(page_id).strip(),
                                "course_id": str(course_id),
...
def update_unit_tab_name(
    body: openapi_body(UpdateBankUnitNameRequest, {"tab_name": "新名稱"}),
    caller_person_id: PersonId,
    course_id: CourseId,
    bank_page_id: str = PathParam(..., description="要更新 tab_name 的 bank_page_id"),
):
    """
...
async def create_upload_zip(
    ...
    try:
        create_result = _create_bank_record(
            bank_page_id=fid,
            person_id=pid,
            tab_name=name,
            course_id=course_id,
...
@router.post("/pages/{bank_page_id}/build-zip")
def build_bank_zip(
    body: openapi_body(
        PackRequest,
        {
    ...
    caller_person_id: PersonId,
    course_id: CourseId,
    bank_page_id: str = PathParam(..., description="來源上傳 ZIP 的 bank_page_id"),
    ...
    """
    依先前上傳的 ZIP（path 參數 bank_page_id）與 unit_list 重新打包。
    **FAISS 建置規則（逐 unit 判斷）**：`user_type==1`（且未強制關閉）且該 unit 之 `unit_type==1 (rag)` → 建 FAISS 並上傳至 rag；`unit_type` 為 2／3／4 時仍 repack 原 ZIP，但 **rag 區上傳內含單一 `transcript.md`（逐字稿全文）之 ZIP**，非 repack 複製；其餘 unit_type==0 等 → repack 同內容複製至 rag。
    ...
    LLM API Key 僅在「最終會建 FAISS」（do_rag 為 True）時必填（依 course_id 自 Course_Setting key=bank-api-key 取得；見 PUT /v1/bank/llm-api-key）。
    body.unit_types 為選填，與 unit_list 逗號分段對齊；**未傳或該段為 0** 時會依單元 ZIP 推斷（恰一音訊＋一文字檔→3、僅一個 .md 等→2；**YouTube 仍須明確傳 4**）。寫入各 Bank_Unit.unit_type。**推斷為 2** 且來源為 `.md`/`.txt` 時 **Bank_Unit.transcript** 為檔案 UTF-8 全文（含 Markdown）。
    ...
    body.unit_names 為選填，與 packed 任務同序（逗號字串或 JSON 字串陣列）；該段非空白時覆寫串流 output.unit_name 與寫入之 Bank_Unit.unit_name（顯示名）。output.folder_combination 恒為 repack ZIP 檔名 stem（寫入 Bank_Unit.folder_combination；多資料夾為 ``a/tb/tc``）。
    ...
    **回應為 NDJSON 串流**（`application/x-ndjson`），請以 `fetch` 讀取 `response.body`，勿使用單次 `response.json()`。
    ...
    整批成功時自動在 Bank_Unit 表建立對應記錄（每個輸出單元一筆）並更新 Bank.rag_metadata。
    整批任一有 `rag_error` 則 `complete.success` 為 false（不寫入 Bank 表，不建立 Bank_Unit）。

    事件列舉（每行一個物件）：
    - `{"type":"start","total":N,"source_bank_page_id":"...","unit_list":"...","user_type":int,"build_faiss_request":bool|null,"repack_only":bool,"allow_faiss":bool}`（allow_faiss=各 unit 是否可建 FAISS，仍需 unit_type==1 才實際建）
    - `{"type":"building","index":i,"total":N,"completed_before":i-1,"filename":"..."}`
    - `{"type":"unit",...,"output":{...}}`：output 含 **folder_combination**（單元 repack ZIP 檔名 stem，寫入 Bank_Unit.folder_combination；多資料夾為 ``folder1/tfolder2``）、**unit_name**（顯示名，可經 unit_names 覆寫）、rag_mode（`faiss`＝向量庫；`transcript_md`＝逐字稿 md ZIP；`repack_copy`＝與 repack 同內容複製）、`transcript_plain`（鍵名沿用舊版；**unit_type=2 且來源為 .md/.txt 時為檔案 UTF-8 全文，Markdown 原樣**，與寫入 Bank_Unit.transcript 一致）；**text_file_name** 僅 **unit_type=2** 有值（來源文字檔檔名）；**mp3_file_name** 僅 3；**youtube_url** 僅 4；**rag_chunk_size**、**rag_chunk_overlap**（本任務實際使用，與 Bank_Unit 一致）；rag_filename（物件鍵仍為 *_rag.zip）
    - `{"type":"complete","success":bool,"total","built_ok","built_failed","source_bank_page_id","unit_list","outputs"}`
    ...
    def _do_rag_for_unit(ut: int) -> bool:
        ...
        return allow_faiss and (ut == BANK_UNIT_TYPE_RAG)
            ...
            yield _ndjson_line(
                {
                    "type": "start",
                    "total": total,
                    "source_bank_page_id": body.bank_page_id,
...
@router.get("/pages/{bank_page_id}/units")
def list_bank_units(
    _caller_person_id: PersonId,
    course_id: CourseId,
    bank_page_id: str = PathParam(..., description="要列出 Bank_Unit 的 bank_page_id"),
):
    """
    依 bank_page_id 列出所有未刪除的 Bank_Unit。
    依 created_at 由舊到新排序。
    """
    try:
        fid = (bank_page_id or "").strip()
        if not fid:
            raise HTTPException(status_code=400, detail="請傳入 bank_page_id")
        supabase = get_supabase()
...
def _fetch_bank_unit_row_by_id_or_http_error(
    *,
    ...
    """
    依 bank_unit_id＋擁有者查 Bank_Unit 一列並驗證（存在、未刪、bank_page_id 一致、unit_type 相符）。
    ...
    def fetch(cols_base: str):
        def build(with_course_filter: bool):
            cols = select_without_course_id_if_needed("Bank_Unit", cols_base, with_course_filter)
            q = (
                supabase.table("Bank_Unit")
                .select(cols)
        ...
        return execute_with_course_id_fallback("Bank_Unit", build, course_id)
    ...
    if not sel.data:
        raise HTTPException(
            status_code=404,
            detail="找不到該 bank_unit_id，或與此 bank_page_id／擁有者不一致",
        )
    row = sel.data[0]
    if row.get("deleted"):
        raise HTTPException(status_code=404, detail="該單元已刪除")
    if (row.get("bank_page_id") or "").strip() != tab:
        raise HTTPException(
            status_code=400,
            detail="bank_page_id 與該 bank_unit_id 所屬之 Bank_Unit.bank_page_id 不一致",
        )
    try:
...
@router.get(
    ...
    operation_id="bank_tab_unit_mp3_file",
    response_model=BankUnitMp3FileResponse,
)
def bank_tab_unit_mp3_file(
    course_id: CourseId,
    bank_unit_id: int = PathParam(..., gt=0, description="Bank_Unit 主鍵"),
    bank_page_id: str = Query(..., description="Bank.bank_page_id（parent tab；repack/upload 路徑皆在其下）"),
):
    """
    依 bank_page_id 與 bank_unit_id；**僅 Bank_Unit.unit_type=3（音訊單元）** 時回傳原始音訊。
    **不需** query `person_id`；後端依 `bank_page_id` 自 Bank 解析擁有者後讀 Storage。
    **優先**自該單元之 **repack** ZIP（`Bank_Unit.repack_file_name`／Storage `…/repack/{單元}.zip`）內，依 **folder_combination**（無則 **unit_name**）
    路徑段擷取第一個支援的音訊檔（repack 內仍保留上傳時之資料夾名，與 `repack_tasks_to_zips` 一致）。
    repack 無法讀取時**改讀**該 tab 之 **upload** ZIP。
    Storage `…/rag/{tab}_rag.zip` 僅為逐字稿封包，不含原始 mp3。
    """
    owner_pid = resolve_bank_tab_owner_person_id(bank_page_id, course_id)
    tab = (bank_page_id or "").strip()
    row = _fetch_bank_unit_row_by_id_or_http_error(
        owner_pid=owner_pid,
        bank_unit_id=bank_unit_id,
        bank_page_id=bank_page_id,
        course_id=course_id,
        cols_with_folder="bank_unit_id, bank_page_id, unit_name, folder_combination, unit_type, deleted, repack_file_name, transcript, course_id",
        cols_without_folder="bank_unit_id, bank_page_id, unit_name, unit_type, deleted, repack_file_name, transcript, course_id",
        expected_unit_type=BANK_UNIT_TYPE_MP3,
    ...
    return BankUnitMp3FileResponse(
        bank_unit_id=bank_unit_id,
        bank_page_id=tab,
...
@router.get(
    "/units/{bank_unit_id}/text",
    summary="Bank Tab Unit Text",
    operation_id="bank_tab_unit_text",
    response_model=BankUnitTextResponse,
)
def bank_tab_unit_text(
    course_id: CourseId,
    bank_unit_id: int = PathParam(..., gt=0, description="Bank_Unit 主鍵"),
    bank_page_id: str = Query(..., description="Bank.bank_page_id（parent tab）"),
):
    """
    依 bank_page_id 與 bank_unit_id；**僅 Bank_Unit.unit_type=2（文字單元）** 時回傳 `text_file_name` 與 `transcript`（全文，含 Markdown）。
    **不需** query `person_id`；後端依 `bank_page_id` 自 Bank 解析擁有者。
    逐字稿以 `Bank_Unit.transcript` 為準；DB 無逐字稿時改讀該 tab 之 **upload** ZIP（以 `folder_combination` 或 `unit_name` 為資料夾名）。
    """
    owner_pid = resolve_bank_tab_owner_person_id(bank_page_id, course_id)
    tab = (bank_page_id or "").strip()
    row = _fetch_bank_unit_row_by_id_or_http_error(
        ...
        bank_page_id=bank_page_id,
        course_id=course_id,
        cols_with_folder="bank_unit_id, bank_page_id, unit_name, folder_combination, unit_type, deleted, text_file_name, transcript, course_id",
        cols_without_folder="bank_unit_id, bank_page_id, unit_name, unit_type, deleted, text_file_name, transcript, course_id",
        expected_unit_type=BANK_UNIT_TYPE_TEXT,
    ...
    if not transcript and zip_folder:
        try:
            ...
            _logger.debug("GET /bank/units/{id}/text ZIP 備援略過: %s", e)
        except Exception:
            _logger.exception("GET /bank/units/{id}/text ZIP 備援失敗")

    if not transcript.strip():
        raise HTTPException(
            status_code=404,
            detail="該單元無逐字稿（Bank_Unit.transcript 為空，且自 upload ZIP 讀取備援失敗或內容為空）",
        )

    return BankUnitTextResponse(
        bank_unit_id=bank_unit_id,
        bank_page_id=tab,
...
@router.get(
    "/units/{bank_unit_id}/youtube-url",
    summary="Bank Tab Unit Youtube Url",
    operation_id="bank_tab_unit_youtube_url",
    response_model=BankUnitYoutubeUrlResponse,
)
def bank_tab_unit_youtube_url(
    course_id: CourseId,
    bank_unit_id: int = PathParam(..., gt=0, description="Bank_Unit 主鍵"),
    bank_page_id: str = Query(..., description="Bank.bank_page_id（parent tab）"),
):
    """
    依 bank_page_id 與 bank_unit_id；**僅 Bank_Unit.unit_type=4（YouTube 單元）** 時回傳 watch URL 與 `transcript`。
    **不需** query `person_id`；後端依 `bank_page_id` 自 Bank 解析擁有者。
    `youtube_url`／`transcript` 以 `Bank_Unit` 欄位為準；DB 缺值時改讀該 tab 之 **upload** ZIP
    （文字檔第一行為 YouTube URL、第二行起為逐字稿）。
    """
    owner_pid = resolve_bank_tab_owner_person_id(bank_page_id, course_id)
    tab = (bank_page_id or "").strip()
    row = _fetch_bank_unit_row_by_id_or_http_error(
        ...
        bank_page_id=bank_page_id,
        course_id=course_id,
        cols_with_folder="bank_unit_id, bank_page_id, unit_name, folder_combination, unit_type, deleted, text_file_name, youtube_url, transcript, course_id",
        cols_without_folder="bank_unit_id, bank_page_id, unit_name, unit_type, deleted, text_file_name, youtube_url, transcript, course_id",
        expected_unit_type=BANK_UNIT_TYPE_YOUTUBE,
    ...
    if (not youtube_url or not transcript) and zip_folder:
        try:
            ...
            _logger.debug("GET /bank/units/{id}/youtube-url ZIP 備援略過: %s", e)
        except Exception:
            _logger.exception("GET /bank/units/{id}/youtube-url ZIP 備援失敗")
    ...
    if not transcript.strip():
        raise HTTPException(
            status_code=404,
            detail="該單元無逐字稿（Bank_Unit.transcript 為空，且自 upload ZIP 讀取備援失敗或第二行起無內容）",
        )

    return BankUnitYoutubeUrlResponse(
        bank_unit_id=bank_unit_id,
        bank_page_id=tab,
        folder_name=zip_folder,
...
# ---------------------------------------------------------------------------
# GET /bank/pages/{bank_page_id}/unit-preview/*（建置前，Bank_Unit 尚未存在時用）
# ---------------------------------------------------------------------------


def _read_upload_zip_bytes_or_http_error(person_id: str, bank_page_id: str) -> bytes:
...
@router.get(
    "/pages/{bank_page_id}/unit-preview/text",
    summary="Bank Page Unit Preview Text",
    operation_id="bank_page_unit_preview_text",
    response_model=BankUnitTextPreviewResponse,
)
def bank_page_unit_preview_text(
    caller_person_id: PersonId,
    course_id: CourseId,
    bank_page_id: str = PathParam(..., description="Bank.bank_page_id（upload ZIP 路徑）"),
    ...
    **建置前預覽**（Bank_Unit 尚未建立、無 bank_unit_id 時用）：自 upload ZIP 內指定資料夾讀取**恰好一個**文字檔全文作為 `transcript`（unit_type=2，與 build-zip 一致）。
    ...
    return BankUnitTextPreviewResponse(
        bank_page_id=tab,
        folder_name=folder,
        text_file_name=Path(inner_path).name,
        transcript=transcript,
...
@router.get(
    "/pages/{bank_page_id}/unit-preview/mp3-file",
    summary="Bank Page Unit Preview Mp3 File",
    operation_id="bank_page_unit_preview_mp3_file",
    response_model=BankUnitMp3FilePreviewResponse,
)
def bank_page_unit_preview_mp3_file(
    caller_person_id: PersonId,
    course_id: CourseId,
    bank_page_id: str = PathParam(..., description="Bank.bank_page_id（upload ZIP 路徑）"),
    folder_name: str = Query(..., description="upload ZIP 內單元資料夾名"),
...
@router.get(
    "/pages/{bank_page_id}/unit-preview/youtube-url",
    summary="Bank Page Unit Preview Youtube Url",
    operation_id="bank_page_unit_preview_youtube_url",
    response_model=BankUnitYoutubeUrlPreviewResponse,
)
def bank_page_unit_preview_youtube_url(
    caller_person_id: PersonId,
    course_id: CourseId,
    bank_page_id: str = PathParam(..., description="Bank.bank_page_id（upload ZIP 路徑）"),
    folder_name: str = Query(..., description="upload ZIP 內單元資料夾名"),
    ...
    **建置前預覽**：自 upload ZIP 內指定資料夾讀取**恰好一個**文字檔：第一行為 YouTube URL，第二行起為 `transcript`（unit_type=4，與 build-zip 一致）。
    ...
    return BankUnitYoutubeUrlPreviewResponse(
        bank_page_id=tab,
        folder_name=folder,
        youtube_url=f"https://www.youtube.com/watch?v={vid}",
        text_file_name=Path(inner_path).name,
