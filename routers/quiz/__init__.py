"""Quiz（試卷／Test）API 模組。對應 public.Quiz／Quiz_Group／Quiz_QA；定位等同 exam 之於 rag，但搭配 bank。
...
兩個子路由器（皆掛 /quiz）：
1. 試卷／題組／出題／批改／評分（.routes）：
   試卷：GET/POST /quiz/pages → PATCH/DELETE /quiz/pages/{quiz_page_id}
