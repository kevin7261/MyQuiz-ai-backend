...
from routers.bank.helpers import (
    BANK_UNIT_TYPE_MP3,
    BANK_UNIT_TYPE_TEXT,
...
def build_quiz_group_snapshot_row(
    *,
    ...
    return {
        ...
        "qa_count": int(bank_group.get("qa_count") or 0),
...
def quiz_llm_generate_qa_impl(
    *,
    ...
    api_key, llm_model, qup, qsp = _resolve_quiz_llm_params(
        group,
        course_id,
    ...
    try:
        fields = _generate_quiz_qa_fields(
            supabase,
            group,
            course_id=course_id,
            api_key=api_key,
...
def update_quiz_qa_with_answer(
    ...
    *,
    ...
    """背景批改完成後更新 Quiz_QA：寫 answer_content、answer_critique 與 answer_llm_model；prompt 為出題時凍結，不覆寫。回傳 (id_key, id_val)。"""
    if quiz_qa_id <= 0:
        return None
...
async def enqueue_quiz_qa_answer_job(
    ...
    *,
    ...
    llm_model = qa_answer_llm_model or get_quiz_llm_model(course_id)
    ...
    if unit_type in _TRANSCRIPT_UNIT_TYPES:
        ...
        work_dir = Path(tempfile.mkdtemp(prefix="myquizai_quiz_answer_tx_"))
    else:
        ...
        work_dir = Path(tempfile.mkdtemp(prefix="myquizai_quiz_answer_"))
    ...
    def insert_fn(rd, ans):
        return update_quiz_qa_with_answer(rd, ans, quiz_qa_id=quiz_qa_id, answer_llm_model=llm_model)
