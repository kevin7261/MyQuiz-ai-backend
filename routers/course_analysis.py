...
- GET /course-analysis/quizzes：回傳全部 Exam_Quiz，依 exam_tab_id 分群對應 Exam；
  每筆 Exam 的題目結構與 GET /exam/tabs 相同（quizzes[]，Exam_Quiz 含 follow_up 鏈；作答內嵌於各題列）。
"""
...
class ListQuizzesResponse(BaseModel):
    """GET /course-analysis/quizzes 回應。exams[] 每筆與 GET /exam/tabs 相同含 quizzes[]（另含 weakness_report 固定 null）。"""
    exams: list[dict]
    count: int
...
def list_exam_quizzes(_person_id: PersonId):
    """
    回傳 Exam_Quiz 全部內容。
    exams 陣列：每筆 Exam 的 quizzes 形狀與 GET /exam/tabs 一致。
