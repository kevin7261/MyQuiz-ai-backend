...
產生**下一題**（同步、會等 LLM）。用題組既有的 prompt；同題組已出過的題幹會當作「勿重複」一起送進去；並自動併入本題組 `Quiz_Ask` 追問紀錄（提問＋回答）作為出題依據。**前端要呼叫 `qa_count` 次**才出滿一份。
...
只重產這一題的 `question_*` 並覆寫回同一 `quiz_qa_id`（不新增、不改 `question_series_index`、不檢查上限）。prompt 自 `Quiz_Group` 讀取；並自動併入本題組 `Quiz_Ask` 追問紀錄作為出題依據。同題組「此題之前」的題作為勿重複。重出後該題舊的作答／批改／評分會清空。無 Body；回應同 2.6。
