...
- **分析指令**
  - **user-analyses**：不預存於 `Course_Setting`；每次 `POST …/llm-analysis` 在 **body** 傳入 `analysis_user_prompt_text`（可空字串）。`User_Analysis.analysis_prompt_text` 為該次快照。
  - **quiz-analyses**：**不支援**自訂分析指令（無預存、無 body）；LLM 僅依作答資料分析。

### 使用模式（分析結果列）

```
1. POST /{prefix}                     → 建一筆空白分析列，取得 id
2. POST /{prefix}/{id}/llm-analysis   → user-analyses 可帶 body 分析指令；quiz-analyses 不帶 body
...
```json
{
  "analysis_user_prompt_text": "請依本次作答分析我的弱點並提出建議…"
}
```

| 欄位 | 必填 | 說明 |
|------|------|------|
| `analysis_user_prompt_text` | 否 | 本次分析指令；預設 `""`（LLM 僅依作答資料分析） |
...
|------|------|------|
| `person_id` | ✅ | 呼叫者登入帳號（須與該列 owner 相符） |
| `course_id` | ✅ | 課程 ID（須與該列相符；取 `quiz-api-key` / `quiz-llm-model`） |
...
```
PUT /v1/quiz/llm-api-key
PUT /v1/quiz/llm-model
```
...
2. POST /v1/user-analyses/{user_analysis_id}/llm-analysis
        ...
        Body: { "analysis_user_prompt_text": "請分析我的弱點…" }
...
2. POST /v1/quiz-analyses/{quiz_analysis_id}/llm-analysis
        ...
        → 取得 weakness_report（Markdown，彙整全課程學生答題狀況；無自訂指令）
...
| 分析指令 | Course_Setting（GET/PUT prompt） | Course_Setting（GET/PUT prompt） | **每次 llm-analysis body** | **無**（僅依作答資料） |
| Course_Setting prompt key | `person_analysis_user_prompt_text` | `course_analysis_user_prompt_text` | — | — |
...
| `course_id` | bigint | 課程 |
| `analysis_name` | varchar(255) | 分析名稱 |
| `analysis_prompt_text` | text | 保留欄位，程式不使用 |
