...
> 本文件兩支 API 分析 **Bank 出題測驗（Quiz_QA）**；**user-analyses** 金鑰為 `user-analysis-api-key`、模型為 `user-analysis-llm-model`；**quiz-analyses** 金鑰為 `quiz-analysis-api-key`、模型為 `quiz-analysis-llm-model`。
...
管理者／教師預先設定個人弱點分析指令（寫入限 user_type 1／2）。
...
管理者／教師預先設定測驗課程分析指令（寫入限 user_type 1／2）。
...
| 分析對象 | 個人 (RAG 題) | 整門課 (RAG 題) | 個人 (Bank 測驗題) | **整門課**（全體學生 Bank 測驗題） |
